...
public class Engine {
    ...
    public static void main(String[] args) throws IOException, CsvException, EPCompileException, EPDeployException, InterruptedException {
        ...
        // String epl = "@name('Pattern') select a.px as priceA, b.px as priceB, (a.px-b.px) as priceDiff from pattern [every a=Event(company=\"A\") -> b=Event(company=\"B\")];";
        // 分别监控A和B最近2次的价格平均值，计算两者均值的价格差
         String epl = "insert into A select avg(px) as price, company from Event(company=\"A\").win:length(3);\n" +
        ...
        deployment.getStatements()[deployment.getStatements().length-1].addListener(new Listener());
